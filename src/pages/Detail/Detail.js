...
function Detail() {
    ...
    const { id } = useParams(); // 현재 라우팅된 페이지의 url 파라미터 추출 
    const matchId = allMovie.find((movie) => { return movie.id == id }); // 현재 페이지 id 파라미터와 일치하는 영화 데이터 가져와서 matchId에 저장 

    // 최근본작품 기능 구현
    useEffect(() => {
        // 현재 페이지 localStorage 저장하기
        let recent = localStorage.getItem('watched'); // let으로 선언해야 값 읽기 및 변경 가능 
    ...
    const handleClick = () => {
        ...
        if (onclick) {
            dispatch(deleteLocker(matchId)); // store에 이미 추가되었다면, 데이터를 store에서 삭제
        } else { 
            dispatch(addLocker(matchId)); // 그렇지 않으면, 데이터를 store에 추가
        }
    };
